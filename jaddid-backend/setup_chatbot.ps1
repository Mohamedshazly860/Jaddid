...
if (Test-Path "env\Scripts\Activate.ps1") {
    ...
    Write-Host "  Warning: Virtual environment not found at env\Scripts\Activate.ps1" -ForegroundColor Yellow
...
if (-not (Test-Path ".env")) {
    ...
    Write-Host ""
    Write-Host "  IMPORTANT: Please add your OpenAI API key to .env file" -ForegroundColor Yellow
    ...
    if ($envContent -match "OPENAI_API_KEY=sk-") {
        ...
        Write-Host "  OPENAI_API_KEY not configured in .env" -ForegroundColor Yellow
